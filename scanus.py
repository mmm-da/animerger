...
def get_subtitles_from_templates(path,template_dict):    
    ...
    for child in dir.iterdir():
        if(child.is_dir()):
            result_dict.update(
                get_subtitles_from_templates(child,template_dict)
            )
        else:
            if child.suffix.lower() in subtitles_files_extensions:
                if child.stem in result_dict:
                    subtitle_entity = [str(child),Path(str(child)+'/..').resolve().stem,'rus',0]
...
def get_audio_from_templates(path,template_dict):    
    ...
    for child in dir.iterdir():
        if(child.is_dir()):
            result_dict.update(
                get_audio_from_templates(child,template_dict)
            )
        else:
